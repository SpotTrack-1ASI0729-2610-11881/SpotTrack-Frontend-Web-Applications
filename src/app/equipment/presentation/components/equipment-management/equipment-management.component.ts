...
export class EquipmentManagementComponent {
  ...
  openAddDialog(): void {
    const ref = this.dialog.open(AddEquipmentDialogComponent, {
      data: null,
      panelClass: 'eq-dialog-panel',
      backdropClass: 'eq-dialog-backdrop',
    });
    ref.afterClosed().subscribe((result: EquipmentFormData | undefined) => {
      if (!result) return;
      const newItem: EquipmentRow = {
        ...result,
        id: `EQ-${String(this.allEquipment().length + 1).padStart(3, '0')}`,
      };
      this.allEquipment.update(list => [...list, newItem]);
    });
  }

  openEditDialog(equipment: EquipmentRow): void {
    const ref = this.dialog.open(AddEquipmentDialogComponent, {
      data: { ...equipment },
      panelClass: 'eq-dialog-panel',
      backdropClass: 'eq-dialog-backdrop',
    ...
    ref.afterClosed().subscribe((result: EquipmentFormData | undefined) => {
      ...
      this.allEquipment.update(list =>
        list.map(eq => eq.id === result.id ? { ...eq, ...result } as EquipmentRow : eq)
      );
    });
  }
